"""Remove duplicate WS callback blocks; keep one clean copy."""
import subprocess
...
def to_function_body(block_lines: list[str]) -> str:
    return "\n".join("    " + ln if ln else "" for ln in strip_min_indent(block_lines))


voice_body = to_function_body(orig_lines[43738:43841]).replace("requested_voice_lang", "lang")
voice_body = "    try:\n" + "\n".join(
    ("        " + ln[4:] if ln.startswith("    ") and ln.strip() else ln)
    for ln in voice_body.splitlines()
) + "\n    except RuntimeError as re:\n        logger.debug(\"%s WebSocket closed: %s\", conn_id, re)\n"
text_stripped = strip_min_indent(orig_lines[44087:44436])
...
text_body = text_body.replace("continue", "return")
text_body = text_body.replace(
    "session_language = msg_lang  # update session preference",
    "session_language = msg_lang\n        session_language_ref[0] = msg_lang",
)

# kb-events block from git (line 44493+)
kb_start = 44492  # 0-indexed line 44493
kb_block = "\n".join(orig_lines[kb_start:44591])  # through asr-status removed, keep preflight area
# asr was 44562-44575 - include preflight from patched file later
...
# Cut from TTS endpoint comment stub through before kb-events real handler
start_marker = "# ========== TEXT-TO-SPEECH ENDPOINT (proxies to XTTS v2 microservice) =========="
end_marker = '@app.websocket("/ws/kb-events")'
...
tail = text[end:]
text = text[:start] + start_marker + "\n#\n" + callbacks_and_ws + tail
SRC.write_text(text, encoding="utf-8")
print("deduplicated callbacks")
